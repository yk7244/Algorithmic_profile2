-- TubeLens Supabase Database Schema
...
CREATE TABLE public.cluster_analysis (
  ...
  analysis_data JSONB NOT NULL, -- 클러스터 분석 결과
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE public.image_data (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
  cluster_id UUID REFERENCES public.cluster_analysis(id) ON DELETE CASCADE,
...
-- ============================================
-- 7. 히스토리 테이블 (무드보드 변경 이력)
-- ============================================
CREATE TABLE public.moodboard_history (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
...
-- 클러스터 분석 관련 인덱스
CREATE INDEX idx_cluster_analysis_user_id ON public.cluster_analysis(user_id);
CREATE INDEX idx_cluster_analysis_created_at ON public.cluster_analysis(created_at DESC);
...
-- 히스토리 관련 인덱스
CREATE INDEX idx_moodboard_history_user_id ON public.moodboard_history(user_id);
CREATE INDEX idx_moodboard_history_created_at ON public.moodboard_history(created_at DESC);
...
-- 클러스터 분석 테이블 RLS
ALTER TABLE public.cluster_analysis ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage own cluster analysis" ON public.cluster_analysis FOR ALL USING (auth.uid() = user_id);
...
-- 히스토리 테이블 RLS
ALTER TABLE public.moodboard_history ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can manage own history" ON public.moodboard_history FOR ALL USING (auth.uid() = user_id);

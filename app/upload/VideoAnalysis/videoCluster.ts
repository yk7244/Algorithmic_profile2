...
(1) 구체적인 콘텐츠 관심 흐름과 고유명사
(2) 시청 성향과 목적
(3) 내면 가치 및 감정 흐름에 대해 깊이 있게 이해할 수 있는 전문가입니다.
제공된 클러스터를 분석해 응답형식에 맞게 친절히 설명해주세요

핵심키워드의 프레임: [흥미 대상]에 [행동/성향]을 가진 [사람형 표현] (예: 도시 관찰러, 밈 헌터)
단, (1) 구체적인 고유명사 기반으로 기억에 남는 흥미대상을 표현할 것, 
(2) 사람들에게 공감이 되고 구체적인 관심사를 드러내는 세련되고 참신한 사람형 표현을 쓸 것
...
const generateNickname = async (clusters: any[], openai: any) => {
  ...
  const prompt = `
    ...
    추천프레임: [시청성향, 태도] + [구체적인 콘텐츠 관심 분야] 동물
    예시: 진심으로 요리를 사랑한 다람쥐, 고요하게 우주를 탐험하는 고슴도치

    1. 사용자의 대표 관심사를 종합하여 봤을때, 특징적인 취향과 성격을 종합적으로 반영한 독특하고 창의적인 별명생성 (예: "요리를 탐구하는 ", "컷 편집에 빠진", "선율에 잠긴" 등)
    2. 중요!!: 별명 생성시 재밌는 동물, 식물, 과일등의 생명체로 은유법이나 비유 명사를 무조건 활용해야함 ("예: 현아를 좋아하는 사과, 토끼)
    3. 사용자의 콘텐츠 소비 패턴, 취향, 관심사를 2-3문장으로 구체적으로 표현하고 짧게 재밌게 흥미롭게 요약한 설명, 사용자를 예측해도 됨

    ...
    <main className="min-h-screen ">
      ...
      <div className="max-w-6xl mx-auto p-8 mt-20">
        ...
        <div className="flex items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-black"> 탐색 결과 </h1>
        </div>
        {show && (
        <div className="fixed top-18 right-10 bg-white/90 backdrop-blur-lg text-black px-7 py-3 rounded-full shadow-xl flex items-center min-w-[420px] max-w-[600px] z-50 animate-fadeIn">
          <span className="text-base flex items-center p-2 pr-3 pl-3">
            <img src="/images/cokieIcon.svg" alt="click" className="w-4 h-4 mr-4" />
            비슷하지만 새로운 알고리즘 정체성 키워드, 발견해보기 <br/>
            끌리는 키워드는 내 자화상에도 담을 수 있어요.
          </span>
          <button
        ...
        <div className="mb-4 flex flex-row items-center gap-2">
          <div className="flex flex-wrap gap-3">
            {keywords.map((keyword, index) => (
              <div 
                key={index}
                className="bg-black/10 backdrop-blur-md px-4 py-2 rounded-full "
              >
        ...
        <div className="mt-1">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-black text-xl"> 당신과 비슷한 취향의 사람을 찾고 있어요...</p>
            </div>
          ) : searchResults.length > 0 ? (
            <>
            <h2 className="text-lg text-black/80  font-bold mb-4">와 비슷한 알고리즘 자화상을 
            <span className="text-blue-500">{searchResults.length}개 </span> 발견했어요</h2>

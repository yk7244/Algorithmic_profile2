...
CLUSTER_END`;
    ...
    const clusters = response.split('CLUSTER_START')
      ...
      .map((cluster: any) => {
        ...
        // 각 라인에서 키와 값을 정확히 추출
        const parsedData = lines.reduce((acc: any, line: any) => {
        ...
        const relatedKeywords = parsedData.keywords ? 
          parsedData.keywords.split(',').map((cluster: any, k: string) => k.trim()).filter(Boolean) : 
          [];
        ...
        return {
          ...
          metadata: {
            ...
            moodKeywords: (parsedData.mood_keyword || '').split(',').map((cluster: any, k: string) => k.trim()).filter(Boolean)
          }
        };
      })
      .filter((cluster: any) => cluster.related_videos && cluster.related_videos.length >= 3);
